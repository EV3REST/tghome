...
from telegram import ForceReply, InlineKeyboardButton, InlineKeyboardMarkup, ParseMode, ReplyKeyboardMarkup, KeyboardButton, InlineQueryResultArticle, InlineQueryResultPhoto
...
def random():
	...
	track_id = randint(0,trackcount-1)
...
def track():
	...
	global p
	try:
		file = "tracks/%s" % tracklist[track_id]
	except:
		try:
			track_id-=1
			file = "tracks/%i" % tracklist[track_id]
		except:
			track_id+=1
			file = "tracks/%i" % tracklist[track_id]
